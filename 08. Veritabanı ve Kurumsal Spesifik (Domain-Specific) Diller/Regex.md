...
"İçinde `@` İşareti Var mı Diye Bak... Varsa Ardından `.com` harflerini Ara... Yok O Varsa Uzunluğunu ölç". Bu hem 50 satırlık İğrenç Bir Spagetti Kod Doğururudu, hem de `ali@@.com..com` Giren adamı O Kodla Yakalayamazdınız Sistemi Patlatıp Sızardı.
...
* **Kazı / Sıyırma (Scraping):** 10 Bin sayfalık HTML / Web sitesi İndiriniz. Sisteminizden Sadece "05XX ile Başlayan Telefon Numaralarını" Çekip Ayıklamak İstiyorsanız; Yüzlerce (Split) Bölümü yazmak yerine 1 Satırlık Regex yazarsınız, Pıt diye O Datanın İçinden Milyonlarca Telefon verişini Önünüze Tabağa Dizer. Karadelik gibi Süzücü ve Sihirlridir.
...
4. `*` : Kendinden Önceki (Örn: A*) Kuralından "Sıfır yada Binlerce, Sınırsız var!" demektir. (Asteriks).
...
Sıradan gibi Görünen O E-Posta şablonunu (user@domain.com) ; Dünyadaki En Güvenli, C#, Java, Python dilleerinde 1 Satırda Adamın girdiği epostayı Hacklenemez Kılan Doğrulama Formülü Şudur (`/ /` Kapanları arasına yazılır):
...
* `[a-zA-Z\d\.-]+` ==> (Domain Kısmı: gmail, hotmail gibi): Sadece Harf, Rqkam, Çizgi olabilir ve Yine en az 1 karakter olmak ZOrunlu!
* `\.` ==> Uzantı Başlıyor Kaçış (Yani Gerçek Özel bir (.) Nokta gellll!). (Özel Karakterin Özeliğini Kaçmak Slash\)
* `[a-zA-Z]{2,6}` ==> Uzantı (.com, .tr, .co.uk): SADECE Harf olmak Zorundadır ve MİNİMUM 2 HARF; MAXİMUM 6 HARF Uuzunlğunda (.online vb) kilitli Limitlidir! Uzun olursa Kabul Etme.
