...
Stuart Feldman Make'i icat edip dedi ki: **"Bir Taktik Tahtası (Makefile) yazalım. Make, Hangi dosyanın ne zaman Değiştirildiğinin (Zaman damgası - Timestamp) kaydını tutsun. Eğer 100 dosyadan SADECE 1'i değişmişse, Gidip sadece o 1 Tanesini derlesin, Sonra diğer 99'uyla eski hallerini Birleştiriverin (Link)!"**
Bu vizyon, Derleme Süresini 20 dakikadan 2 saniyeye düşürerek Bilişim Tarihini hızlandırdı.

## Ne İşe Yarar?
* **Dependency Graph (Bağımlılık Ağacı) Çözme:** `A.c` dosyasının derlenmesi için `A.h`ye ihtiyaç varsa, Make bu ağacı Hiyerarşik olarak kurar ve Hata yapmadan doğru sırayla derler.
...
2 Tane C Kodu dosyamız var (`main.c` ve `matematik.c`). Bunlardan Bir "Uygulama.exe" Çıkarma reçetesi Şöyledir:
...
HEDEF_PROGRAM = uygulamamız # Cikacak Olan Programin(Exe) Adi

# 2. ANA HEDEF (KOK KURAL): 'all' (Terminale sadece 'make' yazdigimizda Oynayacak kural)
# Uygulamamiz programinin cikarilabilmsi için ÖNCE (main.o ve matematik.o) dosyalarina ihtiyacı vardir!
all: $(HEDEF_PROGRAM)

# 3. BAGLANTI (LINKING) AŞAMASI: Nasil Olusacak?
# hedef: bagimlilik1 bagimlilik2 
$(HEDEF_PROGRAM): main.o matematik.o
	# TAB TUSU ILE BASLANAN KOMUT SATIRI: Gcc yi cagir, O iki ".o" (Obje) dosyasini alip Birleştir
	$(CC) $(CFLAGS) -o $(HEDEF_PROGRAM) main.o matematik.o

# 4. OBJELERI TEK TEK DERLEME KURALLARI
main.o: main.c
	# Sadece main.c'yi derleyip Ara-Format olan "main.o" ya çevir! (C işareti compile etmektedir)
	$(CC) $(CFLAGS) -c main.c

matematik.o: matematik.c fonksiyonlar.h
	# Eger matematik.c VEYA icindeki Kütüphane(fonksiyonlar.h) son 1 saatte Degistirilmisse burayi TETIkLE:
	$(CC) $(CFLAGS) -c matematik.c

# 5. TEMIZLIK KURALI (Ortaligi Toparlama)
clean:
	# Terminale 'make clean' yazildiginda Çıkan Bütün Çöpleri(.o dosyalari ve exe yi) Sil:
...
* Tarihi değeri paha biçilemez ve hala her gün Aktif çalışan ender Yarı-Asırlık dillerden biridir. Mükemmeldir lakin o Tab/Boşluk sıkıntısıyla C++ da kan kusturtur ve Yerini yavaş yavaş (CMake)'e Bırakmaya başlamıştır.
