...
if(Mermi_Carpti_Mi) {
    ...
    if(Can <= 0) { Die(); }      // Eger Olduyse YokEt fonksyonu cagir
}
```
...
* Hayatında Tek Satır Kod Görmeden Unreal Engine indirerek kendi Başlarına Mükkemmel FPS Piyasası/Korku(Outlast gibi) oyunlar yapan ve Satış rekoru  Kıran Bağımsız Yapımcılar. 
* *Blueprint Spagettisi:* Kötü Mimar olanlar o kadar çok Kablo çekerler ki, Ekran Mavi ve kırmız renkle Dolanmış İğrenç Bir Spagetti(Kablo Çorbası) Makarnasına döner (Programlamadaki Spagetti kodun Fiziksel Enjeksiyonudur). Lakin günümüzde Dünya Grafik ve Simulasyonlarının Kalbi Görsel Programlamadan Geçmektedir.
