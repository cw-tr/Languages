...
Macromedia/Adobe "Flash Player" Eklentisi yaptı. İnterent Kullanıcılarına dedi ki "Şu 2 Megabytel'lik Yamayı Kur!". Ve BAM! Bir anda Bütün İnternet sitelerinin ortasındaki Siyah Kutuda; ÇöpAdam Oyunları (Kraloyun/Miniclip), Sesli Chat siteleri, MSN Messenger Animasyonları belirdi. Zamanında İnternet demek; Oynatılıyorsa "Flash" Demekti.
...
ActionScript 3; tamamen ECMAScript standardında yapısını Kuran (JavaScript ve C#'ı andıran), Mükkemmel derecede Sıkı Objeye-Yönelik (Class, Public/Private) yapıya sahip bir Şaheserdi. 
...
Ekranda (Sahnedeki/Stage) Duran bir Oyuncuyu (Örn: Çöp Adam) Sağ(Right) Yön tuşuyla haraket ettiren O 2006 Kokulu AS3 Kodu:
...
// Stage(Global Oyun Sahnesi), Klavyeden Bir Tuşa (KEY_DOWN) basildiysa -> OyuncuyuHaraketEttir Fonksiyonunu tetikle!
stage.addEventListener(KeyboardEvent.KEY_DOWN, OyuncuyuHaraketEttir);
...
function OyuncuyuHaraketEttir(event:KeyboardEvent):void {
    
    // Eger Basilann Tus (Eventin Icindeki Tusa Basma Kodu == Klavyedeki SAG(RIGHT) Ok Tusu İse):
    if (event.keyCode == Keyboard.RIGHT) {
        
        // Ekranda (Sahne Hiyerarsisinde) kendi cizdiyim ÇöpAdam'ın Ismini (MC_CopAdam) cagrip
    ...
    // Eger Kullanici BOŞLUK (SPACE) Tysubab Bastiysa?
        ...
        var tufekSesim:Soud = new TufekSesi(); // OOP Mucizesi : Yeni nesne uret
        tufekSesim.play();
...
2. **2010 Steve Jobs Deklerasyonu "Thoughts On Flash":** Apple'ın CEO'su Steve Jobs, "Flash; Çok Pil/Şarj harcıyor, mobil telefonları kasıyor, Dokunmatik (Touch) için değil Fare (Mouse/Tıklama) için tasarlandı ve BÜGÜLÜ. BİZ YENI cıkan iPHONE (DÜNYA DEVRİMİNE) FLASH PLAYER'I BİLEREK YÜKLEMEYECEĞİZ!" Şeklinde devasa o açık mektubu yayınladı. Dünyada Satılan Milyarlarca Cep Telerefonunda Flash Oyunları Açılmayınca, Sistem felç oldu.
