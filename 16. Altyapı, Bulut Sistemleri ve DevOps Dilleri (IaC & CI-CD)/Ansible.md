...
Zavallı Sistem Yöneticisi saatler boyunca Putty (SSH) Üzerinden 100 Makineye Tek Tek Şifreleriyle Bağlanıp 100 kere `apt-get install yama` yazmak zorundaydı. Biri çöker, Birinin Bağlantısı Düştü mü Karmaşa çıkardır.

Ansible Dedi ki: Ban Bir **YAML formatında OyunKitabı (Playbook)** ver. Makine Liste(İp Adreslerini - Inventory) ver. Sonra Tıkla! Parola vs Girerek Arka Planda Ben(Ansible) Cihazlara "Gönüllü ajan(SSH)" olarak Sızıp Hepsine O 1 Komutu ayna Anada Senkronize Şuttlayacağım.
...
2. **Tasks (Görevler):** O Hedefler Ne(Hangi işlevi / Modülü) Yapacaak? (Örn Yükle, Sil, İndir, Git-Clone Çek)
...
- name: Bütün Web Sunucularima (1000 Tane Cihaza) Apache Yükleme Operasyonu
  hosts: web_sunucularim      # Inventory (Ip listesi) dosyasinda Bu isimki Grubu Hedef Al!
  become: yes                 # Root (SuperUser / Muteakil İlah) Yetksiyle Islem Yap! (Sudo yetkisi al)
  ...
  tasks:
    ...
    - name: 1. İslem -> Apache (Httpd) Paketnamesinin İndirildiğibden Emin Ol !! 
      dnf:                    # Linux'un DNF/YUM Paket Yoneticisi Modulune Baglaniyoruz!
        name: httpd           # İndrilecek Paketin Adi!
    ...
    - name: 2. Islem ->  Apache Servisini Tamamen Actigindan / Oynattignindan Emin OL!
      service:                # Ansible'daki Arkapalan (Daeemon/Servces) Modulu 
        name: httpd
        state: started        # Calistir(Start)!
        enabled: yes          # Cihaz CokuP Resest atilirsa Tekrar otomatik Baslasin mi? YES!
        
        
    # GOREV 3: Web Sitesi İcerigine Benim Ozel Dosyami Atmakk (Copylama)
    - name: 3. Islem -> Sitenin Ana Ekarina(Index.html'e) Ozel Bir Hosgeldin Yazisi Kopyala Gonder!
      copy:                   # Bilgisayatlar arasi Kopyalama Modylü
        content: "<h2>Ansible Tarafından Butun Dunyayya Isik Hizinda Automatik Gonderildi!</h2>"
        dest: /var/www/html/index.html   # Adamin Makinesindeki (Hedef Cihz) Gidecegbi Klasor Yolu!
```
